...
* Datum: 2026-04-20
* Status: Aktive Fassung. Loest `CONCEPT_v3.md` als Diskussionsgrundlage ab. v1, v2, v3 bleiben als eingefrorene Referenz erhalten.
...
Gegenueber `CONCEPT_v3.md` gelten folgende grundlegende Aenderungen:

* **Namensauftrennung geklaert.** Drei Namen, drei Rollen, nach der Umentscheidung vom 2026-04-20:
  - **AURUM** = Live-Version (Monolith-HTML, frozen). Gleichzeitig der Konzept-Name fuer die App als Ganzes.
  - **CUPRUM** = Werkstatt (React + Vite + TypeScript). Hier passieren alle Aenderungen. Wird zum neuen AURUM live, sobald reif.
  - **ARGENTUM** = Dark-Mode-Theme **innerhalb** der AURUM-App. Kein eigener Track mehr.
* **Stack-Wechsel** vollzogen. v3 plante Capacitor plus PWA auf einer HTML-Datei. v0.1 hat den React-Port bereits begonnen und fuehrt ihn unter CUPRUM weiter. Die Capacitor-Schicht bleibt Zielbild, ist aber nachgelagert.
* **UI-Muster konkret.** Erste umsetzungsreife Spezifikationen fuer Scrollrad-Picker, Onboarding, Planned-Sheet, Week-Dots, Calendar-Sheet liegen in `documentation/ui/`. Dieses Konzept verweist sie und beschreibt das gemeinsame Gestaltungssystem.
* **Archiv als Kernbestandteil.** Soft-Delete plus 60-Tage-Cleanup sind Teil des Konzepts, nicht nur Implementierungsdetail.
* **Admin-Modus** ist weiter Kernbestandteil, mit neuer Rolle: erlaubt manuelle Anker-Erstellung in der App, solange der Armreif fehlt.

Alles, was v2 und v3 ueber Leitidee, zwei Ebenen, Begriffssystem der vier Marker-Typen, Timeline-Funktion und AURA-Zurueckhaltung gesagt haben, bleibt inhaltlich unveraendert.
...
### 5.4 Geplanter Marker

Kein klassischer Reminder. Er ersetzt die Push-Notification durch einen graduellen koerperlichen Reiz am Armreif. Seit v0.04:
...
* **v0.09** | erster React-Port als damaliger ARGENTUM-Track. AURUM-treue Timeline, MarkerDetail-Modal, Notes-Editor, Settings mit allen AURUM-Sections plus Archiv, Mobile-BottomNav plus FAB, Schema-Migration `archived_at`, Force-Auth-Screen-Fix, Keine sichtbare AI
* **v0.1 (CUPRUM)** | Namensauftrennung geklaert (AURUM live, CUPRUM Werkstatt, ARGENTUM Theme), Subdomain `cuprum.marcodemont.ch`, Starter `CUPRUM.bat`, `vite.config.ts` `allowedHosts` fuer Tunnel, UI-Spezifikationen in `documentation/ui/`

Aktueller Fokus: Stabilisierung von v0.1 als Werkstatt und Portierung der AURUM-Monolith-Features nach CUPRUM.
...
16. **Schema-Aenderungen sind shared infrastructure.** AURUM live und CUPRUM nutzen dieselbe DB. Migrationen muessen backward-kompatibel sein.
...
**Admin-Modus** | lokaler Sichtbarkeits-Schalter fuer Debug- und Entwicklungs-Elemente. Default: an. Erlaubt manuelle Anker-Erstellung solange der Armreif fehlt.
...
* ein zeitbasiertes Selbstwahrnehmungssystem mit drei Namen fuer drei Rollen (AURUM live, CUPRUM Werkstatt, ARGENTUM Dark-Mode-Theme) und AURA als stiller KI-Schicht im Hintergrund
...
* mit Admin- und Praesentationsmodus fuer Diplomarbeits-Kontext
* mit Geraete-Reset fuer neutralen Neustart ohne Datenverlust
* mit dokumentierten UI-Mustern fuer Scrollrad, Onboarding, Planned-Sheet, Week-Dots, Calendar-Sheet
