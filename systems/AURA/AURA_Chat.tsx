...
export function AuraChatBot({
  ...
  const generateAuraResponse = (query: string): string => {
    ...
    if (q.includes("hrv") || q.includes("herzfrequenz")) {
      return "**HRV-Dokumentation gefunden**\n\n**Heart Rate Variability (HRV)**\n\nHRV misst die Variation der Zeit zwischen aufeinanderfolgenden Herzschlägen:\n\n**Normale Werte:**\n- Erwachsene: 50-100ms RMSSD\n- Sportler: >100ms RMSSD\n\n**Einflussfaktoren:**\n- Stress: Verringerte HRV\n- Erholung: Erhöhte HRV\n- Schlaf: Erhöhte HRV\n- Training: variabel\n\n**Referenzen:**\n- FNN Layer 1: Fractal Decomposition\n- Baseline-Algorithmus: 30-Tage-Fenster\n- Schwellwerte: Patient-spezifisch\n\n**Hinweis:** Für detaillierte Analysen nutzen Sie bitte das Arzt-Portal.";
    }

    if (q.includes("fnn") || q.includes("fractal")) {
      return "**FNN-Architektur Übersicht**\n\n**Fractal Neuro Network (FNN) - 3-Layer-Architektur:**\n\n**Layer 1: Fractal Decomposition**\n- Deterministisch\n- Regelbasiert\n- Reproduzierbar\n- Keine AI-Entscheidungen\n\n**Layer 2: Deterministic Analysis**\n- Schwellwert-Prüfung\n- Pattern Recognition\n- Baseline-Vergleich\n- Konfidenz-Berechnung\n\n**Layer 3: AI Explanation (Optional)**\n- OpenAI für Übersetzung\n- KEINE Entscheidungsgewalt\n- Nur natürlichsprachliche Erklärung\n\n**Wichtig:** Alle medizinischen Entscheidungen erfolgen in Layer 1+2 (deterministisch).\n\n**Referenz:** FNN_neu Architektur-Dokumentation";
    }

    if (q.includes("konflikt") || q.includes("log")) {
      return "**System-Logs - Letzte 24h**\n\n**Konflikte:** Keine\n**Warnungen:** 2\n**Info-Events:** 47\n\n**Letzte Warnungen:**\n\n1. **[12:34]** Baseline-Schwellwert unterschritten\n   - Patient: Demo-User\n   - Metrik: HRV RMSSD\n   - Wert: 42ms (Baseline: 50ms)\n   - Status: Arzt informiert\n\n2. **[09:15]** API-Latenz erhöht\n   - Endpoint: /fnn/analyze\n   - Latenz: 245ms (Ziel: <200ms)\n   - Status: Behoben\n\n**Hinweis:** Für detaillierte Logs nutzen Sie das Admin-Panel.";
    }
    ...
    if (
      ...
      return "**Suchergebnisse in Wissensbasis**\n\n**Gefundene Dokumente:**\n\n**FNN_neu_Architektur.md**\n   → Beschreibung der 3-Layer-Architektur\n   → Zuletzt geändert: vor 2 Tagen\n\n**HRV_Baseline_Algorithm.md**\n   → Baseline-Berechnung und Schwellwerte\n   → Zuletzt geändert: vor 5 Tagen\n\n**LYNA_Admin_Guide.md**\n   → Admin-Funktionen und Berechtigungen\n   → Zuletzt geändert: vor 1 Tag\n\n**AURA_ReadOnly_Access.md**\n   → AURA Capabilities und Einschränkungen\n   → Zuletzt geändert: heute\n\n**Frage:** Welches Dokument möchten Sie näher betrachten?";
    }
    ...
    >
      ...
      <div className="bg-gradient-to-r from-cyan-100 to-blue-100 px-4 py-3 border-b border-cyan-200">
        <div className="flex gap-2 flex-wrap">
          ...
          <button
            onClick={() =>
              handleQuickAction("Erkläre FNN-Architektur")
            }
            disabled={isLoading}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-cyan-50 text-cyan-700 rounded-lg text-xs transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
          >
          ...
          <button
            onClick={() =>
              handleQuickAction("Suche HRV-Dokumentation")
            }
            disabled={isLoading}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-cyan-50 text-cyan-700 rounded-lg text-xs transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
          >
